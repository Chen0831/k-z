...
Server_Speeder_installation_status(){
	[[ ! -e ${Server_Speeder_file} ]] && echo -e "${Error} 没有安装 (Server Speeder)，请检查 !" && exit 1
}
...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/aiastia/k-z/master/bbr.sh; then
...
Set_config_password(){
	...
	stty erase '^H' && read -p "(默认: aiastia):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="aiastia"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/aiastia/k-z/master/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/aiastia/k-z/master/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/aiastia/k-z/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf banall.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/aiastia/k-z/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf banall.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
